...
class LVMOGP_GP(GP_gpflow):
    ...
    def build_model(self, seed=None, heteroskedastic_inputs=False, heteroskedastic_outputs=True, sparse=False, n_u=100,
        ...
        #
        # H_mean_init = ops.pca_reduce(tf.convert_to_tensor(lmc_means, dtype=default_float()), self.lvmogp_latent_dims)
        # H_var_init = tf.ones((len(lmc_means), self.lvmogp_latent_dims), dtype=default_float())*1e-4
        #
        ...
        #                                                 kernel=kernel_H,
        #                                                 num_inducing_variables=len(list(self.lmc.coregion_levels.values())[0]),
        #                                                 X_prior_var=tf.ones(
        #                                                     (len(list(self.lmc.coregion_levels.values())[0]), self.lvmogp_latent_dims))
        #                                                 )
        #             if priors:
        ...
        #             linear_variances = [lin_var+1e-4 for lin_var in linear_variances]
